...
In the context of
**Agent-to-Agent (A2A)**
and API governance, these tags serve as machine-readable metadata within an
**Agent Card**
or API specification. They allow agents to autonomously discover, filter, and safely interact with one another while adhering to enterprise compliance rules.
...
Capability tags describe the specific actions or
**skills**
an agent can perform. This allows orchestrator agents to match a complex task to the right specialized sub-agent.
...
Sensitivity tags are the backbone of
**automated governance**
and
**policy enforcement**
. They inform the system whether an agent is allowed to handle certain types of data or perform high-risk actions.
...
These tags are typically embedded in the
**Agent Card JSON**
, allowing a client agent to perform a
GET
request to a
.well-known
URI and immediately understand the target's domain, what it can do, and what security constraints apply.
Are you looking to implement these tags in a specific
**API gateway**
(like Sensedia or IBM API Connect) or directly within an
**A2A Agent Card**
?
...
Mintlify
 +4
Below are samples showing how to structure this JSON to include Domain, Capability, and Sensitivity tags for automated governance. 
1. Sample Agent Card: Finance Payroll Agent
This example shows a highly regulated agent that handles sensitive data within a specific business vertical.
json
{
...
2. Sample Agent Card: Corporate Travel Assistant
This sample demonstrates a lower-sensitivity agent that focuses on task delegation and orchestration.
json
{
...
For consistent governance, these tags are often standardized across an organization:
...
API Gateways: Platforms like Sensedia or IBM API Connect can use these tags to filter traffic or enforce security policies.
Agent Registries: Orchestrator agents (like a "Chief of Staff" agent) query these cards to delegate work to the correct specialized sub-agent. 
Composio
 +3
Would you like to see how these tags can be used to automatically generate security policies or filter agents in a discovery registry?
...
## Answer:
The short answer is:
**The structure of the "tags" field is standardized**
**, but the specific values (the actual text) are often flexible.**
In the
**Agent-to-Agent (A2A) Protocol**
, the
**Agent Card**
is a standardized JSON document exposed at
/.well-known/agent-card.json
. While the protocol defines
*where*
these details go, it often leaves the specific taxonomy of the tags to the organization or community implementing them.
...
The specific values you mentioned (like
HCM
,
PII
, or
...
or
**ontology**
rather than a rigid global standard.
...
If the tags were
*only*
plain text with no structure, an agent couldn't reliably automate security.
...
Are you trying to align your agent tags with a specific industry standard like
**HIPAA**
or
**GDPR**
...
## Answer:
You’ve hit on the exact evolution of how agents talk to each other!
You are correct: In an
**Agent Card**
, "Skills" function much more like
...
- **What it is:** A structured JSON entry (like the samples I showed) that defines a **specific execution endpoint**.
- **How it works:** Just like an **MCP Tool**, it defines:**Name/ID:** create_invoice**Parameters:** What data is needed (e.g., amount, currency).**Governance Tags:** (e.g., domain: finance, sensitivity: PII).
...
## How MCP fits in
If you are familiar with Anthropic’s
**Model Context Protocol (MCP)**
, the "Skills" in an Agent Card are essentially the
**manifest**
of what tools that agent is exposing to the world.
...
## The "Hybrid" Reality
In many A2A implementations, the Agent Card provides the
**structured metadata**
(the MCP-style tool definition), but it might also include a
description_url
that points to a
skill.md
or documentation page for the LLM to use as "long-form reasoning" guidance.
Are you looking to bridge a library of existing
**markdown-based skills**
into a more
**governed, tool-based architecture**
?
