...
Yes,
**OpenClaw**
supports A2A (Agent-to-Agent) communication, and
it generally provides or enables
**Agent Cards**
for each agent instance through specific plugins or external tools
.
## A2A Agent Cards in OpenClaw
In the A2A protocol (originally developed by Google), an
**Agent Card**
is a JSON manifest that describes an agent's identity, URL, and skills, acting like a "digital business card" for discovery.
...
**Are you looking to connect your OpenClaw agents to each other, or are you trying to link them to an external framework like LangGraph?**

---

## Question:
when I have an card of an existing agent what can I do with that in openclaw?

## Answer:
In OpenClaw, once you have an
**Agent Card**
(a JSON manifest describing a remote agent's capabilities and URL), you can
use it to enable your local OpenClaw assistant to
**delegate tasks**
to that external specialist
.
Here is what you can specifically do with an Agent Card in the OpenClaw environment:
## 1. Remote Task Delegation (A2A Client)
The most common use is setting up your local OpenClaw instance as an
**A2A Client**
. You can register the card's URL to give your personal agent the ability to "outsource" work.
...
With tools like
**NEXUS**
(a community-built coordination layer for OpenClaw), an Agent Card allows your agent to join a "Yellow Pages" for AI.
...
**Would you like the specific CLI commands to register a remote agent card as a new skill in your current workspace?**
...
LumaDock
 +1
Are you setting this up for a single personal bot, or do you need to manage a fleet of agents across different machines?
