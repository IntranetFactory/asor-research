# Claude Plugin Agents, Subagents, and Tool Integration — Q&A

> Source: sources/can I define an agent or subagent with a claude plugin.md
...
## Answer:
Yes, you can define specialized agents (commonly called subagents) within a Claude plugin to perform focused tasks. When you bundle subagents into a plugin, they become portable, allowing you to share specialized workflows across different projects or teams without manually copying configuration files.
...
## Answer:
The primary difference is their **role in the workflow**: a **Skill** is a piece of modular knowledge or a specific "how-to" recipe that Claude follows, while an **Agent** (or subagent) is a separate, autonomous worker that operates in its own isolated workspace.
...
## When to Use a Skill
Use a **Skill** when you want Claude to get better at a specific, repeatable task without leaving the main conversation.
...
Use an **Agent** when a task is large, requires extensive searching, or needs a specialized role.
...
## Answer:
Claude plugins are quite flexible — they are **not** limited to MCP tools. You can reference **locally installed CLIs**, bundled scripts, and even Language Server Protocol (LSP) servers within a single plugin. For many developer workflows, using a CLI is often better than an MCP server because it reduces context bloat and doesn't require maintaining a separate background process.

## 1. Referencing Locally Installed CLIs
You can use **Skills** or **Subagents** to teach Claude how to use any CLI already on your machine (like `git`, `docker`, or your own custom tools).
...
**Pro Tip:** If your CLI outputs a massive amount of data, it's often better to have the agent run a command that filters the output (like `my-tool | grep "error"`) rather than reading the whole log, which saves your context window.
